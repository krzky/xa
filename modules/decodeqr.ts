...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, XA: XA, args: string[]): Promise<void> {
                ...
                qrcode.callback = async function (err: string | string[], value: { result: any; }) {
                    if (err) {
                        ...
                        if (err.includes('enough finder patterns')) {
                            await client
                                .sendMessage(XA.chatId, Strings.decodeqr.INVALID_INPUT, MessageType.text);
                        }
                    } else {
                        await client
                            .sendMessage(XA.chatId, value.result, MessageType.text)
                            .catch((err) =>
        ...
        try {
            if (!XA.isTextReply || (XA.isReplyAudio || XA.isReplyVideo || XA.isReplyAnimatedSticker)) {

                await client
                    .sendMessage(XA.chatId, DECODE.INVALID_REPLY, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, XA));
                return;

            } else if (XA.isReplySticker) {

                processing = await client
                    .sendMessage(XA.chatId, DECODE.PROCESSING, MessageType.text);
                var replyChatObject = {
                ...
                await client
                    .sendMessage(XA.chatId, DECODE.INVALID_INPUT, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, XA));
                return;
